...
program
  ...
  .action((d2Path: string, storyPath: string, opts: { out?: string; viewerBundle?: string; theme?: string }) => {
    ...
    // Check d2 is available before attempting render
    try {
      execSync("d2 --version", { stdio: "pipe" });
    } catch {
      console.error("Error: d2 is not installed or not on PATH.");
      console.error("");
      console.error("Install d2:");
      console.error("  macOS / Linux:  curl -fsSL https://d2lang.com/install.sh | sh -s --");
      console.error("  Windows:        winget install terrastruct.d2");
      console.error("  Homebrew:       brew install d2");
      console.error("  Docs:           https://d2lang.com/tour/install");
      process.exit(1);
    }
...
program
  ...
  .action((d2Path: string, opts: { out?: string }) => {
    ...
    const lines = [
      ...
      `# See: https://diascope.biolytics.ai/reference/story-format`,
      ``,
      `meta:`,
