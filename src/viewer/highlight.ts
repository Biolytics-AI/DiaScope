...
import type { SvgPanZoom, Step } from "./types.js";
...
export function applyHighlight(viewer: HighlightContext, nodeIds: string[]): void {
  const allNodes = Array.from(viewer.doc.querySelectorAll(".d2-node")) as HTMLElement[];
  const allEdges = Array.from(viewer.doc.querySelectorAll(".d2-edge")) as HTMLElement[];
  ...
  if (viewer.focusMode) viewer.svgHost?.classList.add("focus-mode");
  else viewer.svgHost?.classList.remove("focus-mode");

  allNodes.forEach((n) => {
    const id = (n as HTMLElement & { dataset: DOMStringMap }).dataset["nodeId"] ?? "";
    if (active.has(id)) {
      n.classList.add("lit");
  ...
  // Promote ancestor nodes of lit nodes
  allNodes.forEach((n) => {
  ...
  allEdges.forEach((e) => {
    const el = e as HTMLElement & { dataset: DOMStringMap };
    const ancestorLit = Boolean(e.closest(".d2-node.lit"));
    const src = el.dataset["edgeSrc"];
    const dst = el.dataset["edgeDst"];
    ...
    (e as HTMLElement).style.opacity = viewer.focusMode ? "" : viewer.contextEdgeOpacity;
  });
}

export function autoZoom(viewer: HighlightContext, nodeIds: string[]): void {
  if (!viewer.pz) return;
  if (!nodeIds || nodeIds.length === 0) {
    viewer.pz.fit();
    viewer.pz.center();
  ...
  if (!viewer.canvasWrap) return;

  const active = new Set(nodeIds);
  const litEls = Array.from(viewer.doc.querySelectorAll(".d2-node")).filter(
    (n) => active.has((n as HTMLElement & { dataset: DOMStringMap }).dataset["nodeId"] ?? "")
  ) as HTMLElement[];
  ...
  if (viewer.zoomRaf) cancelAnimationFrame(viewer.zoomRaf);
  ...
  const tick = () => {
    ...
    viewer.pz?.pan({ x: startPanX + (targetPanX - startPanX) * t, y: startPanY + (targetPanY - startPanY) * t });
    if (frame < totalFrames) viewer.zoomRaf = requestAnimationFrame(tick);
  };
